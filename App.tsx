...
import { Activity, Code, BookOpen, Server, BrainCircuit } from 'lucide-react';
        ...
        <header className="flex-none z-50 backdrop-blur-xl bg-slate-950/80 border-b border-white/5 shadow-lg shadow-black/20 h-16 relative">
            ...
            <nav className="hidden md:flex space-x-1">
                ...
                className={`px-4 py-2 rounded-md text-sm font-medium transition-all duration-300 flex items-center space-x-2 ${
                    currentTab === Tab.VISUALIZER ? 'bg-white/10 text-white shadow-[0_0_15px_rgba(255,255,255,0.1)] border border-white/10' : 'text-slate-400 hover:text-white hover:bg-white/5'
                }`}
                >
                <BookOpen size={16} />
                ...
                className={`px-4 py-2 rounded-md text-sm font-medium transition-all duration-300 flex items-center space-x-2 ${
                    currentTab === Tab.PLAYGROUND ? 'bg-white/10 text-white shadow-[0_0_15px_rgba(255,255,255,0.1)] border border-white/10' : 'text-slate-400 hover:text-white hover:bg-white/5'
                }`}
                >
                <Activity size={16} />
                ...
                className={`px-4 py-2 rounded-md text-sm font-medium transition-all duration-300 flex items-center space-x-2 ${
                    currentTab === Tab.PYTHON_LAB ? 'bg-white/10 text-white shadow-[0_0_15px_rgba(255,255,255,0.1)] border border-white/10' : 'text-slate-400 hover:text-white hover:bg-white/5'
                }`}
                >
                <Code size={16} />
            ...
            <div className="flex-grow w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                <AnimatePresence mode="wait">
                    ...
                    >
                        ...
                        {currentTab === Tab.PYTHON_LAB && <PythonLab />}
                    </motion.div>
                </AnimatePresence>
            </div>
